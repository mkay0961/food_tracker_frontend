...
class DetailAddModal extends Component {
  ...
  render() {
    ...
    return (
      <div>
      <Modal dimmer={"blurring"} open={showDetailModal}>
        <Modal.Header>Adding Food</Modal.Header>
        <Modal.Content image>
          <Modal.Description>
            <Header>Item: {(currentModal)?currentModal.name:null}</Header>
            <form className="ui form">
              ...
              <div className="field">
                ...
                <input type="text" name="exp_date" defaultValue={(currentModal)?currentModal.default_expiration:null}/>
              </div>
              <button className="ui button" onClick={(e)=>this.handleDetailsClose(e)}>Update</button>
